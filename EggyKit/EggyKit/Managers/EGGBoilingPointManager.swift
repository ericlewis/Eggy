...
public class EGGBoilingPointManager: EGGBoilingPointManagerProtocol {
    ...
    public weak var delegate: EGGBoilingPointManagerDelegate?
    ...
    public func update(_ data: CMAltitudeData?, _ error: Error?) {
        if let rawPressure = data?.pressure {
            let pressure = Measurement(value: rawPressure.doubleValue,
                                       unit: UnitPressure.kilopascals).converted(to: .inchesOfMercury).value
            let res = Measurement(value: 49.161 * log(pressure) + 44,
                                  unit: UnitTemperature.fahrenheit).converted(to: .celsius).value
